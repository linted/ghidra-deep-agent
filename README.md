...
| `COMPACT_TRIGGER_TOKENS` | *(deepagents default: 170k)* | **Sub-agents:** auto-compact when the full prompt (system prompt + tool schemas + history) reaches this many tokens. Models without a langchain context profile fall back to a 170k trigger that rarely fires — set this (e.g. `50000`) to compact early |
| `COMPACT_KEEP_TOKENS` | *(deepagents default: 6 messages)* | **Sub-agents:** recent history tokens kept verbatim after a compaction (takes precedence over `COMPACT_KEEP_MESSAGES`) |
